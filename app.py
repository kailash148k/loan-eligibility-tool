...
for idx, loan in enumerate(st.session_state.loans):
    with st.container(border=True):
        ...
        l2.info(f"System EMI: ₹{sys_emi:,.0f}")
        ...
        if is_man: active_emi = l2.number_input("Enter Manual EMI", key=f"le_man_{idx}", value=loan.get('man_val', 0.0))
        else: active_emi = sys_emi
